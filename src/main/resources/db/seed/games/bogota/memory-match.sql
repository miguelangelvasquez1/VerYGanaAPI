...
SELECT
		13,
    'Memory Match',
    'Encuentra todos los pares de cartas iguales. Pondrás a prueba tu memoria con imágenes de tu marca.',
    'PATH',
    'memory-match',
    'https://games.verygana.com/game_icons/bogota/memoria.png',
     true, NOW(), NOW()
...
SELECT
  ...
  "type": "object",
  "title": "Memory Card Game Configuration",
  ...
  "properties": {
    ...
    "game_config": {
      "type": "object",
      "title": "Game Config",
      "description": "Core gameplay parameters for the memory card session",
      "required": [],
      "properties": {}
    },
    "game": {
      "type": "object",
      "title": "Game Assets",
      "description": "Card face images used in the memory matching game",
      "required": ["card_images"],
      "properties": {
        "card_images": {
          "type": "array",
          "title": "Card Images",
          "description": "List of card face images. Each image will be used as a matching pair.",
          "minItems": 1,
          "items": {
            "type": "object",
            "title": "Card Image",
            "description": "A single card face entry",
            "required": ["url"],
            "properties": {
              "url": {
                "type": ["string", "null"],
    ...
    "audio": {
      ...
      "properties": {
        ...
        "victory_url": {
          ...
          "description": "URL of the sound played when all pairs are matched",
          "format": "uri",
          "default": ""
        }
      }
    ...
    "rewards": {
      ...
      "properties": {
        "keys_per_action": {
          ...
          "description": "Keys awarded each time the player matches a pair",
          "default": 0,
          "minimum": 0,
          "maximum": 10000
        },
        "keys_on_completion": {
          ...
          "description": "Keys awarded upon matching all pairs",
          "default": 0,
          "minimum": 0,
          "maximum": 100000
        }
  ...
  "game": {
    ...
    "ui:description": "Upload one image per unique card face. The game will create a matching pair for each.",
    "card_images": {
      "ui:title": "Card Images",
      "ui:description": "Add one entry per unique card face image.",
      "items": {
        "url": {
          "ui:widget": "assetUpload",
          "ui:options": { "assetType": "image" },
          "ui:help": "Card face image used as a matching pair"
        }
      }
    }
  },
  "audio": {
    ...
    "victory_url": {
      "ui:widget": "assetUpload",
      "ui:options": { "assetType": "audio" },
      "ui:help": "Sound played when all card pairs have been matched"
    }
  },
  "texts": {
    "ui:title": "UI Texts",
    ...
    "victory_messages": {
      ...
      "items": {
        "ui:widget": "textInput",
        "ui:placeholder": "e.g. ¡Encontraste todos los pares!",
  ...
  "rewards": {
    ...
    "keys_per_action": {
      ...
      "ui:placeholder": "0",
      "ui:help": "Keys granted each time the player matches a pair"
    },
    "keys_on_completion": {
      "ui:widget": "numberInput",
      "ui:placeholder": "0",
      "ui:help": "Keys granted upon matching all pairs"
    }
  }
}',
    true,
...
WHERE NOT EXISTS (
    ...
    WHERE game_id = 13 AND version = 1
);