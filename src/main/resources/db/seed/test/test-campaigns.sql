...
-- version se fija en 0 explícitamente: la entidad Campaign usa @Version
-- (optimistic locking) y Hibernate no la backfillea en filas insertadas
-- por SQL crudo, lo que produce NPE en el primer UPDATE si queda NULL.
-- ============================================================
...
SELECT 1, 1, 1, JSON_OBJECT('type', 'video_view', 'duration', 30), @commercial_id,
    ...
    25000000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), 1, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 2, 2, 1, JSON_OBJECT('type', 'video_view', 'duration', 25), @commercial_id,
    ...
    18000000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 25 DAY), 2, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 3, 3, 1, JSON_OBJECT('type', 'video_view', 'duration', 20), @commercial_id,
    ...
    14000000, 0, 6,
    NOW(), DATE_ADD(NOW(), INTERVAL 20 DAY), 3, 'ACTIVE', NOW(), NOW(), 0
...
INSERT INTO campaigns (id, game_id, config_definition_id, config_data, commercial_id,
    ...
    budget_cents, spent_cents, max_session_per_user_per_day,
    start_date, end_date,
    target_audience_id, status, created_at, updated_at, version)
SELECT 4, 4, 1, JSON_OBJECT('type', 'video_view', 'duration', 15), @commercial_id,
    35.0, 218750, 245000, 192500,
    9800000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 18 DAY), 4, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 5, 5, 1, JSON_OBJECT('type', 'video_view', 'duration', 30), @commercial_id,
    ...
    30250000, 0, 4,
    NOW(), DATE_ADD(NOW(), INTERVAL 35 DAY), 5, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 6, 6, 1, JSON_OBJECT('type', 'video_view', 'duration', 20), @commercial_id,
    ...
    12160000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 22 DAY), 6, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 7, 7, 1, JSON_OBJECT('type', 'video_view', 'duration', 25), @commercial_id,
    ...
    21600000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 28 DAY), 7, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 8, 8, 1, JSON_OBJECT('type', 'video_view', 'duration', 18), @commercial_id,
    ...
    15960000, 0, 6,
    NOW(), DATE_ADD(NOW(), INTERVAL 15 DAY), 8, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 9, 9, 1, JSON_OBJECT('type', 'video_view', 'duration', 28), @commercial_id,
    ...
    24960000, 0, 4,
    NOW(), DATE_ADD(NOW(), INTERVAL 32 DAY), 9, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 10, 10, 1, JSON_OBJECT('type', 'video_view', 'duration', 22), @commercial_id,
    ...
    19740000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 26 DAY), 10, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 11, 11, 1, JSON_OBJECT('type', 'video_view', 'duration', 24), @commercial_id,
    ...
    23000000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), 11, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 12, 12, 1, JSON_OBJECT('type', 'video_view', 'duration', 20), @commercial_id,
    ...
    12000000, 0, 6,
    NOW(), DATE_ADD(NOW(), INTERVAL 20 DAY), 12, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 13, 13, 1, JSON_OBJECT('type', 'video_view', 'duration', 27), @commercial_id,
    ...
    27030000, 0, 4,
    NOW(), DATE_ADD(NOW(), INTERVAL 33 DAY), 13, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 14, 14, 1, JSON_OBJECT('type', 'video_view', 'duration', 19), @commercial_id,
    ...
    16770000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 24 DAY), 14, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 15, 15, 1, JSON_OBJECT('type', 'video_view', 'duration', 22), @commercial_id,
    ...
    18860000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 27 DAY), 15, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 16, 16, 1, JSON_OBJECT('type', 'video_view', 'duration', 18), @commercial_id,
    ...
    16280000, 0, 6,
    NOW(), DATE_ADD(NOW(), INTERVAL 21 DAY), 16, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 17, 17, 1, JSON_OBJECT('type', 'video_view', 'duration', 26), @commercial_id,
    ...
    24990000, 0, 4,
    NOW(), DATE_ADD(NOW(), INTERVAL 31 DAY), 17, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 18, 18, 1, JSON_OBJECT('type', 'video_view', 'duration', 23), @commercial_id,
    ...
    21070000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 29 DAY), 18, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 19, 19, 1, JSON_OBJECT('type', 'video_view', 'duration', 21), @commercial_id,
    ...
    20680000, 0, 5,
    NOW(), DATE_ADD(NOW(), INTERVAL 23 DAY), 19, 'ACTIVE', NOW(), NOW(), 0
...
SELECT 20, 20, 1, JSON_OBJECT('type', 'video_view', 'duration', 30), @commercial_id,
    ...
    35960000, 0, 4,
    NOW(), DATE_ADD(NOW(), INTERVAL 40 DAY), 20, 'ACTIVE', NOW(), NOW(), 0
